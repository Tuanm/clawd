...
import { convertCCFormatToInternal, validateServerConfig } from "../mcp-validation";
    ...
    if (path === "/api/app.mcp.import" && req.method === "POST") {
      return (async () => {
        ...
        try {
          ...
          return json({ ok: false, error: `Invalid JSON: ${err.message}` }, 400);
        }
    ...
    if (path === "/api/app.mcp.install" && req.method === "POST") {
      return (async () => {
        ...
        try {
          ...
          return json({ ok: false, error: `Validation failed: ${err.message}` }, 400);
        }
