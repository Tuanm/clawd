...
export class WorkerLoop implements AgentWorker {
  ...
  // from UHUMAN (the Pilot). Threaded into the re-injection loop so reminder
  // prompts can steer the agent away from [SILENT] on Pilot-triggered turns.
  private lastPendingHasPilot: boolean = false;
      ...
      try {
        ...
        if (result.ok && result.pending.length > 0) {
          ...
          try {
            ...
            this.lastPendingHasPilot = result.pending.some((m: any) => m.user === "UHUMAN");
          ...
          if (wlInterrupted) {
            ...
            while (wlResumeInterruptMsgs.length > 0 && this.running) {
              ...
              try {
                ...
                // reply. Without this, resume turns would reuse a stale ts.
                this.lastPendingTs =
  ...
  private buildMixedPrompt(seenNotProcessed: Message[], unseen: Message[]): string {
    ...
    parts.push(
      `\n[REMINDER: Your streaming text output goes to the agentic framework only — the human CANNOT see it. End the turn with reply to deliver a visible response.]`,
    );
    return parts.join("\n");
  }
    ...
    const sectionHeader =
      seenNotProcessed.length > 0 && unseen.length === 0
        ? `# Messages on Channel "${channel}" (continuing)\n\nCONTINUATION REQUIRED — you did not call ${
            isSpaceAgent ? "complete_task" : "reply"
          } last turn.`
  ...
  private buildInterruptPrompt(processingMessages: Message[], newMessages: any[], hadUnsentText = false): string {
    ...
    if (hadUnsentText) {
      parts.push(
        `\n[WARNING: Your previous turn produced text output but did NOT call \`reply\`. The human cannot see your previous response. If you still need to respond to the earlier task, call \`reply\` FIRST before processing the new messages.]`,
      );
    }
    parts.push(
    ...
    return runWithAgentContext(
      ...
      async () => {
        try {
          ...
          // Re-injection tracking: detect whether reply was called this turn.
          // reply unifies "send message" + "mark processed" — so one signal covers
          // both the text-delivery and pending-cleanup concerns.
          // Heartbeat prompts are allowed to skip reply — suppress re-injection for them.
          const isHeartbeatTurn = prompt.startsWith("[HEARTBEAT]");
          let turnReplyHuman = false;
          /** True if reply was called AND produced a visible message (text != "" / "[SILENT]"). */
          ...
          //
          // Provider/model resolution: honor the `memory` block of
          // ~/.clawd/config.json (same config the memory-extraction LLM uses,
          // typically a cheap/fast model). If unset here, Agent falls back
          // to the parent's provider/model — which is the only correct
          // fallback since the runner's hardcoded default may not exist on
          // the parent's provider.
          const _memCfg = loadConfigFile().memory;
          const _memProvider = typeof _memCfg === "object" && _memCfg?.provider ? _memCfg.provider : undefined;
          ...
          try {
            ...
            // Re-injection: if agent did NOT call reply this turn, send
            ...
            if (!turnReplyHuman && !isHeartbeatTurn && !this.wasCancelledByHeartbeat) {
              ...
              const triggeringIsPilot = this.lastPendingHasPilot;
              for (let attempt = 1; attempt <= MAX_REINJECT_ATTEMPTS && !turnReplyHuman; attempt++) {
                ...
                if (turnReplyHuman) {
                  this.log(`Re-injection: agent called reply on attempt ${attempt}`);
                  break;
                }
                if (!this.running || this.wasCancelledByHeartbeat) break;
              }

              // Fallback rescue: loop exhausted without reply. If any text
