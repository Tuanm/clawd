...
/**
 * Returns true when running in a container environment.
 * Container envs are identified by ENV=dev|prod|staging.
 * Used to tune SQLite PRAGMAs (cache_size, mmap_size).
 */
export function isContainerEnv(): boolean {
  const env = process.env.ENV;
  return env === "dev" || env === "prod" || env === "staging";
}
