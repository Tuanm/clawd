...
export async function discoverOAuthMetadata(
  ...
  try {
    ...
    prmError = err instanceof Error ? err.message : String(err);
  }
  ...
  if (authServerUrl) {
    ...
    try {
      ...
      console.warn(
        `[mcp-oauth] Failed to fetch auth server metadata from ${authBase}: ${err instanceof Error ? err.message : String(err)}`,
      );
      return null;
    }
  } else {
    ...
    try {
      ...
      console.log(
        `[mcp-oauth] No OAuth authorization server metadata at ${origin}: ${err instanceof Error ? err.message : String(err)}`,
      );
      return null;
    }
  }
...
async function tryDynamicRegistration(
  ...
  try {
    ...
    console.warn(`[mcp-oauth] DCR at ${endpoint} error: ${err instanceof Error ? err.message : String(err)}`);
  }
  return null;
}
...
export async function refreshOAuthToken(
  ...
  const promise = (async (): Promise<OAuthToken | null> => {
    try {
      ...
      const msg = (err instanceof Error ? err.message : String(err)).split("http")[0]?.trim() || "unknown error";
