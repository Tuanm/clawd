 ...
 * Uses versioned migrations (src/db/migrations/agents-migrations.ts) instead
 * of inline DDL — idempotent and tracked via PRAGMA user_version.
 */
export function initAgentsTable(db: Database): void {
  runMigrations(db, agentsMigrations);
}
...
export function registerAgentRoutes(
  ...
  return (req: Request, url: URL, path: string, bunServer?: any): Response | null => {
    ...
    if (path === "/api/app.agents.list") {
      ...
      // Only expose full token when request carries a valid internal service token
      const internal = isInternalToken(extractToken(req));
      const enriched = agents.map((a: any) => ({
        ...
        worker_token: a.worker_token
          ? internal
            ? a.worker_token
            : `${a.worker_token.slice(0, 4)}${"*".repeat(12)}`
          : null,
      }));
...
function handleAsync(fn: () => Promise<Response>): Response {
  ...
  // Re-throw Response instances (e.g. 400 from parseBody) as-is; log others as 500.
  return fn().catch((err) => {
