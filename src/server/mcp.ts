...
Use this to respond to user messages or send notifications.`,
    inputSchema: {
      ...
      properties: {
        ...
        user: {
          type: "string",
          description: "User ID override (default: UBOT). Use UWORKER-{agent_id} for worker/sub-agents.",
        },
        html_preview: {
          type: "string",
          description:
      ...
      required: ["channel", "text", "agent_id"],
    },
    annotations: {
      readOnlyHint: false,
      destructiveHint: false,
...
The UI will display the file as a clickable attachment with preview for images.`,
    inputSchema: {
      ...
      required: ["content_base64", "filename", "mimetype", "channel"],
    },
    annotations: {
      readOnlyHint: false,
      destructiveHint: false,
...
Use after uploading files with chat_upload_file.`,
    inputSchema: {
      ...
      required: ["channel", "text", "file_ids", "agent_id"],
    },
    annotations: {
      readOnlyHint: false,
      destructiveHint: false,
...
- Reference important message timestamps for future lookup`,
    inputSchema: {
      ...
      required: ["channel", "summary", "from_ts", "to_ts"],
    },
    annotations: {
      readOnlyHint: false,
      destructiveHint: false,
