...
export class SkillManager {
  ...
  private indexSingleSkill(
    ...
    if (autoGenerated !== undefined) {
      // When autoGenerated is explicitly provided (e.g. from saveSkill), include it in the UPSERT
      // so we avoid a separate UPDATE call after INSERT.
      const autoVal = autoGenerated ? 1 : 0;
      this.db.run(
        `INSERT INTO skills (name, description, path, source, tokens, updated_at, use_count, last_used_at, auto_generated)
         VALUES (?, ?, ?, ?, ?, ?, 0, NULL, ?)
        ...
        [name, metadata.description || "", skillPath, source, tokens, Date.now(), autoVal],
      );
    } else {
      // When called from indexDirectory (re-indexing), preserve existing tracking columns.
      // INSERT OR REPLACE would reset them; ON CONFLICT DO UPDATE preserves use_count/last_used_at/auto_generated.
      this.db.run(
        `INSERT INTO skills (name, description, path, source, tokens, updated_at, use_count, last_used_at, auto_generated)
         VALUES (?, ?, ?, ?, ?, ?, 0, NULL, 0)
         ON CONFLICT(name) DO UPDATE SET
  ...
  saveSkill(
    skill: Omit<Skill, "path" | "tokens" | "source"> & { autoGenerated?: boolean },
    ...
    this.indexSingleSkill(skill.name, skillPath, scope, skill.autoGenerated ?? false);
  ...
  deleteSkill(name: string): boolean {
    ...
    try {
      ...
      console.error(`[SkillManager] deleteSkill FS error for '${name}':`, err);
      // fall through to DB cleanup
    }

    // Always clean DB row regardless of FS errors to prevent zombie entries
    this.db.run("DELETE FROM triggers WHERE skill_name = ?", [name]);
  ...
  touchSkill(name: string): void {
    ...
    // Invalidate cache so next getSkill() reflects updated tracking state
    this.cache.delete(name);
  }

  /**
   * Delete auto-generated skills that haven't been used within maxAgeDays.
   * Returns the count of deleted skills.
   */
  cleanupStaleAutoSkills(maxAgeDays = 30): number {
    const cutoff = Date.now() - maxAgeDays * 24 * 60 * 60 * 1000;
    const stale = this.db
      .query(
        `SELECT name FROM skills
         WHERE auto_generated = 1 AND (last_used_at IS NULL OR last_used_at < ?)`,
      )
      .all(cutoff) as Array<{ name: string }>;

    let deleted = 0;
    for (const { name } of stale) {
      if (this.deleteSkill(name)) deleted++;
    }
    return deleted;
  }
