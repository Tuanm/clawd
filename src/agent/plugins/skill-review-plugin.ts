...
export function createSkillReviewPlugin(config: SkillReviewConfig, deps: SkillReviewDeps = {}): { plugin: Plugin } {
  ...
  // ── Pattern Detection (Lightweight Pre-Filter) ───────────────────────────

  function detectRepeatedToolSequences(): Array<{ sequence: string; count: number }> {
    const sequences: Map<string, number> = new Map();

    // Look for 3-tool sequences
    for (let i = 0; i < messageBuffer.length - 2; i++) {
      const m1 = messageBuffer[i];
      const m2 = messageBuffer[i + 1];
      const m3 = messageBuffer[i + 2];

      if (m1.role === "assistant" && m2.role === "tool" && m3.role === "assistant") {
        // Extract tool names from tool results
        // This is simplified - full impl would parse tool_call_ids
      }
    }
    ...
    try {
      ...
      runWithTimeout()
        .then(async (result) => {
          ...
          const skills = parseSkillRecommendations(result.result, projectRoot);

          if (skills.length === 0) {
            console.log(`[SkillReview] Review #${reviewCount}: no skills found`);
            reviewInProgress = false;
            return;
          }
  ...
  const pluginHooks: PluginHooks = {
    ...
    async onToolResult(name: string, result: any, _ctx: PluginContext) {
      ...
      // Check if we should trigger a review (effectiveInterval guards against 0)
      if (toolCallCount > 0 && toolCallCount % effectiveInterval === 0) {
        // Fire-and-forget: review runs in background. Plugin is unregistered on
        // process exit to avoid orphaned reviews.
