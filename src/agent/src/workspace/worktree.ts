...
export async function createWorktree(projectPath: string, agentId: string): Promise<string> {
  ...
  if (isGitRepo(projectPath)) {
    ...
    execSync(`git worktree add -b '${branchName}' '${worktreePath}'`, { cwd: projectPath, stdio: "pipe" });

    // Initialize submodules in the new worktree — roll back on failure
    if (hasSubmodules(projectPath)) {
      try {
        execSync("git submodule update --init --recursive", { cwd: worktreePath, stdio: "pipe" });
      } catch (subErr: any) {
    ...
    try {
      ...
      if (err.message.includes("not found") || err.message.includes("No such file") || err.status === 127) {
        throw new Error(
          `rsync is not installed. Install it (e.g. "apt install rsync" or "brew install rsync") or use a git repository for worktree isolation.`,
        );
      }
      throw err;
    }
