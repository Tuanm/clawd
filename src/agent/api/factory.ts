      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            console.warn(
              `[Provider] Stream JSON parse error: ${parseErr instanceof Error ? parseErr.message?.slice(0, 80) : String(parseErr)} | data: ${data?.slice(0, 120)}`,
            );
          }
        }
      }
...
class AnthropicProvider implements LLMProvider {
  ...
  async *stream(
    ...
    try {
      ...
      // Skip if doneEmitted is already true (e.g., user abort) — we don't want
      // to emit phantom tool_calls after an explicit abort error.
      if (!doneEmitted && toolCallBuffer.size > 0) {
        console.warn(
