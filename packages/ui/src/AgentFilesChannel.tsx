...
/** Claude AI icon for claude-global agents */
function ClaudeIcon() {
  return (
...
/** Card icon based on agent source */
function AgentCardIcon({ source }: { source: string }) {
  if (source === "built-in") return <ClawdAvatar color="hsl(15 63.1% 59.6%)" />;
  if (source === "claude-global") return <ClaudeIcon />;
  return <ClawdAvatar />;
}

export default function AgentFilesChannel() {
  ...
  const loadAgents = useCallback(async () => {
    setError(null);
    try {
      const res = await authFetch(`${API_URL}/api/app.agent-files.list`);
      const data = await res.json();
      if (data.ok) setAgents(data.agents);
      else setError(data.error || "Failed to load agent files");
    } catch (err) {
      setError(String(err));
    } finally {
      setLoading(false);
  ...
  const handleCardClick = useCallback(
    async (name: string) => {
      ...
      setCardLoading(true);
      try {
        const res = await authFetch(`${API_URL}/api/app.agent-files.get?name=${encodeURIComponent(name)}`);
        const data = await res.json();
        if (data.ok && data.agent) {
          setEditName(data.agent.name || name);
          setEditContent(data.agent.content || "");
          setEditEditable(data.agent.editable !== false);
          setIsNew(false);
          setDialogOpen(true);
        } else {
          setError(data.error || "Failed to load agent file");
        }
      } catch (err) {
        setError(String(err));
      } finally {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!AGENT_NAME_RE.test(name)) {
      setError("Invalid name: use letters, numbers, hyphens, underscores only");
      return;
    }
    setSaving(true);
    setError(null);
    try {
      const res = await authFetch(`${API_URL}/api/app.agent-files.save`, {
        method: "POST",
  ...
  }, [editName, editContent, loadAgents]);

  const handleDelete = useCallback(async () => {
    if (!editName || !confirm(`Delete agent file "${editName}"?`)) return;
    setSaving(true);
    setError(null);
    try {
      const res = await authFetch(`${API_URL}/api/app.agent-files.delete?name=${encodeURIComponent(editName)}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.ok) {
      ...
      <div className="message">
        ...
        <div className="message-content">
          ...
          <div className="message-text">
            <p>Manage your global agent files. Click a card to view or edit.</p>
          </div>
          {loading ? (
            ...
            <>
              {agents.length === 0 && (
                <div className="subspace-card-description" style={{ padding: "8px 0" }}>
                  No agent files found. Click below to create one.
                </div>
              )}
              {agents.map((a) => (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  onKeyDown={(e) => e.key === "Enter" && handleCardClick(a.name)}
                  style={cardLoading ? { opacity: 0.6, pointerEvents: "none" } : undefined}
                >
                  <div className="subspace-card-icon">
                    <AgentCardIcon source={a.source} />
                  </div>
                  <div className="subspace-card-content">
                    <div className="subspace-card-title">{a.name}</div>
                    {a.description && <div className="subspace-card-description">{a.description}</div>}
                  </div>
                </div>
              ))}
              {/* Add new agent card */}
              ...
              >
                ...
                <div className="subspace-card-content">
                  ...
                  <div className="subspace-card-description">Create a new agent file</div>
                </div>
              </div>
            </>
          )}
          {saveWarning && <div style={{ fontSize: 12, color: "hsl(30 80% 50%)", marginTop: 8 }}>{saveWarning}</div>}
            ...
            <div className="stream-dialog agent-file-edit-dialog" onClick={(e) => e.stopPropagation()}>
              <div className="stream-dialog-header">
                <div className="stream-dialog-title-row">
                  <h3>{isNew ? "New Agent" : editName}</h3>
                </div>
