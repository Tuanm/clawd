 ...
 * 5. Error.stack debugger frame inspection
 * 6. Function constructor / eval / setTimeout string debugger stripping
 * 7. chrome.csi / chrome.loadTimes normalization
 *
...
(function () {
  ...
  const _chromeH =
    Math.min(Math.max(window.outerHeight - window.innerHeight, 20), 120) || 80;
  ...
  function _adjustedPerfNow() {
    ...
    if (_highWater > 0 && raw > _highWater + 200) {
      // Anomalous jump detected inline — correct immediately
  ...
  const _origPerfNowDesc = Object.getOwnPropertyDescriptor(
  ...
  // Patch Date.now() — same offset, rounded to integer
  const _patchedDateNow = function now() {
    return Math.round(_dateNow.call(_OrigDate) - _offset);
  };
  ...
  const _prepareStackTrace = function (error, stack) {
    const filtered = stack.filter((frame) => {
      const fn = frame.getFunctionName() || "";
      const file = frame.getFileName() || "";
      return !_suspiciousPatterns.some(
        (p) => file.includes(p) || fn.includes(p)
      );
    });
  ...
  _registerNative(_prepareStackTrace, "prepareStackTrace");
  Error.prepareStackTrace = _prepareStackTrace;

  // =========================================================================
  // 6. Debugger trap neutralization
  //
  // Strips `debugger` statements from: Function constructor, eval,
  ...
  function _stripDebugger(code) {
    ...
    _dbgRe.lastIndex = 0;
    return code.replace(_dbgRe, (m) =>
      m.replace(/debugger(?![\w$])\s*;?/, "void 0;")
    );
  }
  ...
  // eval — indirect eval catches most anti-bot patterns
  const _origEval = window.eval;
  const _patchedEval = function (x) {
    return _origEval(_stripDebugger(x));
  };
  _registerNative(_patchedEval, "eval");
  Object.defineProperty(_patchedEval, "name", { value: "eval" });
  try {
