...
const TOOLS = [
  ...
  },
  {
    name: "snapshot",
    description:
      "Get the accessibility tree of the current browser page. Best for understanding page structure and finding element references.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "screenshot",
    description:
      "Capture a screenshot of the entire display. Automatically uploads to the chat server and returns file_id. Use chat_send_message_with_files to share the screenshot in chat.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "observe",
    description:
      "Take a screenshot and analyze it with vision AI. Returns a structured description of what is visible on screen. Use when snapshot() is insufficient (e.g., extension popups, native apps).",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "click",
    description:
      "Click an element. Priority: ref (fastest, uses Playwright CSS selector) > coordinates (x,y with xdotool) > description (slowest, uses vision AI to locate). Only provide ONE of these.",
    inputSchema: {
      type: "object",
      properties: {
        ref: {
          type: "string",
          description: 'CSS selector or Playwright locator (e.g., "button#submit", "text=Connect Wallet")',
        },
        x: { type: "number", description: "X coordinate (use with y)" },
        y: { type: "number", description: "Y coordinate (use with x)" },
        description: {
          type: "string",
          description: "Natural language description of the element (uses vision AI, most expensive)",
        },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        use_xdotool: {
          type: "boolean",
          description: "Use xdotool instead of Playwright (needed for extension popups)",
        },
      },
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        key: {
          type: "string",
          description: "Key name or combo (Playwright format for web, xdotool format for native)",
        },
  ...
  },
  {
    name: "wait",
    description:
      "Wait for a condition: an element to appear (selector), text to be present, or just wait for a duration.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "get_context",
    description:
      "Get current workspace state: active context, control mode (structured/vision), current URL, and hints for next actions.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "window_manage",
  ...
  },
  {
    ...
    description:
      "Pause agent execution and wait for human input (e.g., CAPTCHA, hardware 2FA, manual confirmation). Provides noVNC link for direct browser access.",
    inputSchema: {
      type: "object",
      required: ["reason"],
      properties: {
...
async function handleToolCall(name: string, args: Record<string, any>): Promise<any> {
  switch (name) {
    ...
    case "screenshot": {
      ...
      const chatUrl = (process.env.CLAWD_CHAT_URL ?? "").replace(/\/$/, "");
      if (!chatUrl) {
        cleanup();
        return {
          error: "CLAWD_CHAT_URL not configured.",
          hint: "Ask the operator to configure CLAWD_CHAT_URL in the workspace environment.",
        };
      }
      try {
        ...
        formData.append("file", new Blob([pngData], { type: "image/png" }), basename(path));
        formData.append("channel", "workspace");
        const uploadResp = await fetch(`${chatUrl}/api/files.upload`, {
          method: "POST",
          body: formData,
        ...
        if (!uploadResp.ok) {
          ...
          throw new Error(`Upload failed: ${uploadResp.status} ${errBody.slice(0, 200)}`);
        }
        ...
        return {
          ...
          width,
          height,
          hint: "Screenshot uploaded. Use chat_send_message_with_files with this file_id to share it in the channel.",
        };
      } catch (err: any) {
        return { error: `Screenshot upload failed: ${err.message}`, hint: "Try again or use observe() instead." };
      } finally {
        cleanup();
      }
    }
    ...
    case "scroll": {
      ...
      if (args.selector) {
        await page
          ...
          .first()
          .evaluate(
            (el: Element, [dir, amt]: [string, number]) => {
              el.scrollBy(
...
// Listen on all interfaces so Docker port mapping (-p 127.0.0.1:HOST_PORT:3000) works.
// Auth token ensures only authorized callers can use the API.
