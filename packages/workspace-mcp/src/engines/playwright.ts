...
export async function closeBrowser(): Promise<void> {
  if (cdpSession) { try { await cdpSession.detach(); } catch {} cdpSession = null; }
...
export async function getAccessibilityTree(): Promise<object> {
  ...
  const snapshot = await page.locator('body').ariaSnapshot({ timeout: 10000 }).catch(() => null);
  if (snapshot !== null) return { snapshot }; // Accept empty string as valid (no elements)
  // Fallback: collect key elements via DOM
  const elements = await page.evaluate(() => {
    const sel = 'button,a,input,select,textarea,[role="button"],[role="link"],[role="checkbox"],[role="menuitem"],h1,h2,h3';
    return Array.from(document.querySelectorAll<HTMLElement>(sel)).slice(0, 200).map(el => ({
